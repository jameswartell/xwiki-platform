...
public class ClassesResourceIT extends AbstractHttpIT
{
    ...
    {
        CloseableHttpResponse response = executeGet(buildURI(ClassesResource.class, getWiki()).toString());
        Assert.assertEquals(getHttpMethodInfo(response), HttpStatus.SC_OK, response.getCode());

        Classes classes = (Classes) unmarshaller.unmarshal(response.getResponseBodyAsStream());
